...
export const searchUniversity = async (req, res) => {
    try {
        ...
        const combination = Array.isArray(req.body.combination) ? req.body.combination : [];

        const query = {};
        let majorConditions = {}; 
        let projection = { _id: 0, name: 1, location: 1, majors: 1 , admission_reference: 1, tuition_fee_link: 1};
        ...
        const hasMajorParams = majorName.length > 0 || 
        ...
        if (hasMajorParams) {
            ...
            if (majorName.length > 0) {
                majorConditions.name = { $in: majorName };
            }
        ...
        const resultSearch = await university
            .find(query)
            .select(projection) 
            .exec();
